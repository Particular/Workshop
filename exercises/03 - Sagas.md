...
In the last exercise you published events in the `Sales` and `Finance` verticals. The `Shipping` vertical has components that are subscribed to their events. When the event arrives, the state is stored in the database.
...
A saga is coordinating long-running business processes. The saga you implement, takes decisions based on data that it receives. Imagine a business process that running for days, years even. Some business processes actually never end. Our saga can't keep all that data in memory forever, so it needs to store this data somewhere.
...
**2)** Now we'll add the products. A minor problem is that we can't add `IList<Guid>` for the products as NServiceBus persisters can't map this properly to tables or documents in SQL-Server and/or RavenDB. We need an additional complex type. So create a class called `Product`, and in the `ShippingSagaData` we'll add a property of `IList<Product>` to contain the products. Of course the `Product` class needs to hold the unique id of the products, so we need to add a property for that as well. We'll end up with a class like this.
```
    class ShippingSagaData : ContainSagaData
    {
        public Guid OrderId { get; set; }
...
**4)** Next implement the abstract methods and interfaces. Make sure everything compiles.
...
In this exercise you will map incoming messages to the saga so NServiceBus knows which property to use to find and retrieve the correct saga instance in the database.
