...
NOTE: If the `IsPaymentProcessedYet` property is generated by using the Visual Studio refactoring tools the property will have a internal setter, such as:

```
public bool IsPaymentProcessedYet { get; internal set; }
```

Make sure to remove the `internal` keyword to make it public, otherwise when the `ShippingSagaData` is rehydrated from the storage the property might not be correctly set.
...
NOTE: If the `IsOrderSubmitted` property is generated by using the Visual Studio refactoring tools the property will have a internal setter, such as:
