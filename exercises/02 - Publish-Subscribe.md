...
If you create a class inside a namespace which name ends with "Events" and the name of this class will end with "Event", then NServiceBus will know it's an event.
...
In this exercise we'll create a new event called `PaymentSucceededEvent`. That event will be published by `InitiatePaymentProcessCommandHandler` in the `Divergent.Finance` project.
